...
def optimize(tokens):
    ...
    do_output = False
    pre_loop_sets = collections.OrderedDict()
    for token, value in tokens:
        prev_token = newtokens[-1][0] if newtokens else None
        ...
        if (token == ENDLOOP and newtokens[-1][0] == LOOP and
            ...
            if len(adds) == 1:
                ...
                for k, v in adds.items():
                    if k != 0:
                        src, dest, mul = 0, k, v
                        if src in pre_loop_sets:
                            val = pre_loop_sets[src] * mul
                            newtokens.append((ADD, (dest, val)))
                        else:
                            newtokens.append((MULCOPY, (src, dest, mul)))
                if sets:
                    newtokens.append((ENDIF, None))
                pre_loop_sets.clear()
                adds.clear()
        ...
        if token == LOOP:
            for k, v in sets.items():
                pre_loop_sets[k+shift] = v
        elif token not in (SET, ADD, MOVE):
            pre_loop_sets.clear()
